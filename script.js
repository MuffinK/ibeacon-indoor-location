...
function drawIndoor(data){
    ...
    for (i = 0; i < spaces.length; i++){    
        ...
        // console.log(JSON.stringify(spaces[i].properties));
...
function createGeometry(space){
    ...
    if (space.hasOwnProperty("geometryDTO") && typeof(space.properties) !== 'undefined'){
        if (typeof(space.geometryDTO.collectionPointDTO) !== 'undefined'){
            ...
            if (typeof(space.properties.highway) !== 'undefined'){ // Stairs
            ...
            else if (space.properties.indoor==="step" ){  // Room              
                var geometry = new L.Polygon(pointList, {color: '#efe3d6', weight:2, fillColor: '#fffbef', opacity: 1, fillOpacity:1,  smoothFactor: 1 });
            }   
...
var currentPosition ;
var currentPlace = {roomName: 'D301', building: 'D', level: 3, roomNo:301};
var places = [
    {roomName: 'D301', building: 'D', level: 3, roomNo:301, roomId: 'D301'},
    {roomName: 'D306', building: 'D', level: 3, roomNo:306, roomId: 'D306'},
    {roomName: 'D401', building: 'D', level: 4, roomNo:401, roomId: 'D401'},
    {roomName: '楼梯', building: 'D', level: 3, roomId: 'steps3'},
    {roomName: '楼梯', building: 'D', level: 4, roomId: 'steps4'}
];
const toSameBuilding = (current, target)=>{
    if(current.building == target.building){
        return [toSameLevel(current, target)];
    }else{
        // return [toSameLevel(current, {}), {name: '前进到' + target.building + '楼', isDone: 0}, toSameLevel() ];
    }
};
const toSameLevel = (current, target)=>{
    if(current.level == target.level){
        return [toRoom(current, target)]
    }else{
        return [
            {name: '前进到楼梯', isDone: 0, label: labels[current.level]['楼梯'], level: current.level}, 
            {name: (target.level>currentPlace.level?'上':'下') + '到'+target.level+'楼',isDone: 0,label: labels[target.level]['楼梯'], level: target.level},
            [toRoom(current, target)]
        ]
    }
};
const toRoom = (current, target)=>{
    return [{name: '前进到'+target.roomName, isDone: 0, label: labels[target.level][target.roomNo], level: target.level}]
}
new Vue({
    el: "#app",
    data(){
        return {
            ...
            targetRooms: places,
            target: '',
            routers: [{name: '下楼', isDone: 0}, {name:'到D306', isDone: 0}],
            origin: '',
            routeIndex: 0
        }
    },
    methods: {
        forward(index){
            if(index === 0){
                ...
                this.routers[this.routeIndex].isDone = 2;
                this.routeIndex = this.routeIndex + 1;
            }
            this.routers[this.routeIndex].isDone = 1;
            currentLayerGroup.clearLayers();
            currentLayerGroup.addLayer(levels[this.routers[this.routeIndex].level]);
            this.routers[this.routeIndex].label.openPopup();
        }
    },
    watch: {
        target(newTarget, oldTarget){
            this.routers = R.flatten(toSameBuilding(currentPlace, newTarget));
            this.routeIndex = 0;
            this.forward(0);
        },
        currentPlace(newPlace, oldPlace){

        }
    },
    mounted(){
        map = L.map("map", {  
            center: [22.37155700,113.57022358],  
            zoom: 20, 
            zoomControl: true
        });  
        ...
        var meMarker = L.marker([22.3732457,113.57130540000001], {icon: greenIcon}).addTo(map);
        // map.on('click', function(e) {
        // 		alert("Lat, Lon : " + e.latlng.lat + ", " + e.latlng.lng)
        // });
        // map.dragging.disable();
        // map.touchZoom.disable();
        var compassControl = new L.Control.Compass({
            autoActive: true,
            showDigit: true
        }) ;
        // map.addControl( compassControl);
        var locateMe = ()=>{
            pageControl.qGetCtrlValue('position')
            .then(res=>{
                if(res){
                    var result = gcoord.transform(
                        [ res.lng, res.lat],    // 经纬度坐标
                        gcoord.BD09,                 // 当前坐标系
                        gcoord.WGS84               // 目标坐标系
                    );
                    meMarker.setLatLng([result[1], result[0]]);
                    map.panTo([result[1], result[0]]);
                }
            })
        }
        locateMe();
        // setInterval(()=>{
                    // 	locateMe();
                    // }, 1000);
        compassControl.on('compass:rotated', event=>{
            ...
            if(rose && rose.style && rose.style.transform){
                ...
                rose.style.transform += ' rotate('+ -event.angle+'deg)';
            }
        })
        
        // lcl = L.control.locate({
        //     setView: 'always'
        // }).addTo(map);
        // lcl.start()
